...
from local_head_command_runner import (
...
test_cases_test_failing_command = [
    ...
    ('FooBar', {'exit_on_fail': False, 'with_output': True}),
]
...
def prepare_dirs():
    ...
    yield files_data
    if os.path.exists(files_data['dirs'][0]):
...
def test_run_rsync_up_dir_not_exists(prepare_dirs):
    files_data = prepare_dirs
    dir0 = files_data['dirs'][1]
    dir2 = files_data['dirs'][0] / "dir2"

    runner = LocalHeadCommandRunner(log_prefix="", cluster_name="some_cluster", process_runner=subprocess)
    runner.run_rsync_up(str(dir0) + '/', str(dir2), options_dir)

    assert are_files_the_same(dir0 / 'file0.txt', dir2 / 'file0.txt')
    assert are_files_the_same(dir0 / 'file1.txt', dir2 / 'file1.txt')
    assert are_files_the_same(dir0 / 'dir01' / 'file0.txt', dir2 / 'dir01' / 'file0.txt')


def test_run_rsync_up_dir_exists_files_not_exist(prepare_dirs):
    files_data = prepare_dirs
    dir0 = files_data['dirs'][1]
...
def test_run_rsync_up_dir_exists_files_updated(prepare_dirs):
    files_data = prepare_dirs
    dir0 = files_data['dirs'][1]
    ...
    runner = LocalHeadCommandRunner(log_prefix="", cluster_name="some_cluster", process_runner=subprocess)
    runner.run_rsync_up(str(dir1) + '/', str(dir2), options_dir)

    assert are_files_the_same(dir1 / 'file0.txt', dir2 / 'file0.txt')
    assert are_files_the_same(dir1 / 'file1.txt', dir2 / 'file1.txt')
    assert are_files_the_same(dir0 / 'dir01' / 'file0.txt', dir2 / 'dir01' / 'file0.txt')


def test_run_rsync_up_source_same_as_target(prepare_dirs):
    files_data = prepare_dirs
    dir0 = files_data['dirs'][1]

    runner = LocalHeadCommandRunner(log_prefix="", cluster_name="some_cluster", process_runner=subprocess)
    runner.run_rsync_up(str(dir0) + '/', str(dir0), options_dir)

    assert are_files_the_same(dir0 / 'file0.txt', dir0 / 'file0.txt')


def test_run_rsync_down_dir_not_exists(prepare_dirs):
    files_data = prepare_dirs
    dir0 = files_data['dirs'][1]
    dir2 = files_data['dirs'][0] / "dir2"

    runner = LocalHeadCommandRunner(log_prefix="", cluster_name="some_cluster", process_runner=subprocess)
    runner.run_rsync_down(str(dir0) + '/', str(dir2), options_dir)
...
def test_run_rsync_down_dir_exists_files_not_exist(prepare_dirs):
    files_data = prepare_dirs
    dir0 = files_data['dirs'][1]
    dir2 = _prep_dir(files_data['dirs'][0], 'dir2')
...
def test_run_rsync_down_dir_exists_files_updated(prepare_dirs):
    files_data = prepare_dirs
    dir0 = files_data['dirs'][1]
    dir1 = files_data['dirs'][3]
    dir2 = files_data['dirs'][0] / "dir2"
    ...
    runner = LocalHeadCommandRunner(log_prefix="", cluster_name="some_cluster", process_runner=subprocess)
    runner.run_rsync_down(str(dir1) + '/', str(dir2), options_dir)
...
@pytest.mark.parametrize('rsync_silent', (True, False))
def test_set_rsync_silent(rsync_silent, prepare_dirs):
    set_rsync_silent(rsync_silent)
    assert is_rsync_silent() == rsync_silent


@pytest.mark.parametrize('using_login_shells', (True, False))
def test_set_using_login_shells(using_login_shells, prepare_dirs):
    set_using_login_shells(using_login_shells)
    assert is_using_login_shells() == using_login_shells
