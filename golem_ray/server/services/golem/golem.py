...
class GolemService:
    ...
    async def _create_reverse_ssh_to_golem_network(self) -> Process:
        ...
        """
        text_command = f"ssh -N -R -o StrictHostKeyChecking=no *:{self._gcs_reverse_tunnel_port}:127.0.0.1:8080 proxy@proxy.dev.golem.network"
        ...
        logger.info(
            f"Reverse ssh tunnel from 127.0.0.1:8080 to *:{self._gcs_reverse_tunnel_port} created."
        )
    ...
    async def _add_authorized_key_to_local_node(key):
        ...
        result = await subprocess.create_subprocess_exec(
            "echo", f"{key}", ">>", "~/.ssh/authorized_keys"
        )
